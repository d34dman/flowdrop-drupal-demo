...
for r in graded:
    runs.append(dict(
        ...
        fidelity=num(r['fidelity']), fabrication=num(r['fabrication']),
        glyphs=int(r['glyphs'] or 0) if r['glyphs'] else None, leaks=int(r['leaks']) if r['leaks'] else None,
        tag=r['tag']))
pausedj = [dict(v=short(r['variant']), page=r['page'], model=MNAME[r['model']], cost=num(r['cost_usd'], 0), tag=r['tag']) for r in paused]
tot = Counter(r['cls'] for r in runs)
spend = sum(r['cost'] for r in runs)
D = dict(runs=runs, paused=pausedj, models=[MNAME[m] for m in MODELS], variants=sorted({r['v'] for r in runs}, key=lambda v: int(v[1:])),
         vname=VNAME, totals=tot, n=len(runs), spend=round(spend, 2))
...
'''
html = html.replace('__DATA__', json.dumps(D, separators=(',', ':'))).replace('__N__', str(len(runs))).replace('__NP__', str(len(pausedj)))
