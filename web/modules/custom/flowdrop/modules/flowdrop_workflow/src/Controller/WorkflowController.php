...
final class WorkflowController extends ControllerBase {
   ...
   */
  public function enableWorkflow(FlowDropWorkflow $flowdrop_workflow): RedirectResponse {
    try {
      $flowdrop_workflow->setStatus(TRUE);
      $flowdrop_workflow->save();

      $this->messenger()->addStatus($this->t('Workflow %label has been enabled.', [
        '%label' => $flowdrop_workflow->label(),
      ]));
    }
    catch (\Exception $e) {
      $this->messenger()->addError($this->t('Failed to enable workflow %label: @message', [
        '%label' => $flowdrop_workflow->label(),
        '@message' => $e->getMessage(),
      ]));
    }

    return new RedirectResponse(Url::fromRoute('entity.flowdrop_workflow.collection')->toString());
  }
   ...
   *   Redirect back to the workflow list.
   */
  public function disableWorkflow(FlowDropWorkflow $flowdrop_workflow): RedirectResponse {
    try {
      $flowdrop_workflow->setStatus(FALSE);
      $flowdrop_workflow->save();

      $this->messenger()->addStatus($this->t('Workflow %label has been disabled.', [
        '%label' => $flowdrop_workflow->label(),
      ]));
    }
    catch (\Exception $e) {
      $this->messenger()->addError($this->t('Failed to disable workflow %label: @message', [
        '%label' => $flowdrop_workflow->label(),
        '@message' => $e->getMessage(),
      ]));
    }
